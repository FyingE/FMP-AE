...
def combined_loss_function(reconstructed, original, matrix_profile, predicted, true, lambda_m=1, lambda_c=1):
...
def detect_anomalies_logical_and(recon_error, matrix_profile, threshold_recon_error, threshold_matrix_profile):
    predicted_labels_recon_error = (recon_error > threshold_recon_error).astype(int)
    predicted_labels_matrix_profile = (matrix_profile > threshold_matrix_profile).astype(int)
    predicted_labels = predicted_labels_recon_error & predicted_labels_matrix_profile
    return predicted_labels


def detect_anomalies_combined_score(recon_error, matrix_profile):
    softmax_matrix_profile = F.softmax(torch.tensor(matrix_profile), dim=0).numpy()
    anomaly_score = softmax_matrix_profile * recon_error^2
    threshold = np.percentile(anomaly_score, 90)  # 调整阈值百分位数
    predicted_labels = (anomaly_score > threshold).astype(int)
    return predicted_labels


def train_and_evaluate(params, method='logical_and'):
    ...
    # 读取示例文件，获取时间序列数据
    file_path_example = 'UCR_Anomaly_FullData/008_UCR_Anomaly_DISTORTEDCIMIS44AirTemperature4_4000_5549_5597.txt'
    ...
    threshold_recon_error = np.percentile(recon_error, 90)  # 调整阈值百分位数
    threshold_matrix_profile = np.percentile(test_matrix_profile, 90)  # 调整阈值百分位数

    # 异常检测
    if method == 'logical_and':
        predicted_labels = detect_anomalies_logical_and(recon_error, test_matrix_profile, threshold_recon_error, threshold_matrix_profile)
    elif method == 'combined_score':
        predicted_labels = detect_anomalies_combined_score(recon_error, test_matrix_profile)
...
for params in grid:
    for method in ['logical_and', 'combined_score']:
        accuracy, precision, recall, f1 = train_and_evaluate(params, method=method)
        score = f1  # 或者根据你的具体需求选择其他指标
        results.append((params, method, accuracy, precision, recall, f1))
        if score > best_score:
            best_score = score
            best_params = (params, method)

print(f"Best Parameters and Method: {best_params}")
print(f"Best Score: {best_score}")

# 打印所有结果
for result in results:
    print(f"Params: {result[0]}, Method: {result[1]}, Accuracy: {result[2]}, Precision: {result[3]}, Recall: {result[4]}, F1: {result[5]}")
