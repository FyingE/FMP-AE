...
from model import create_windows, TimeSeriesCNN, Autoencoder, plot_loss_values, \
    ...
    plot_matrix_profile, plot_reconstruct_error, combined_loss_function, plot_roc_curve, \
    plot_pr_curve
...
# 获取时间序列数据
file_path_example = 'UCR_Anomaly_FullData/028_UCR_Anomaly_DISTORTEDInternalBleeding17_1600_3198_3309.txt'
...
# 创建训练数据窗口，使用反射填充
train_data_windows = create_windows(train_data, window_size=window_size, padding="reflect")
train_data = torch.tensor(train_data_windows, dtype=torch.float32).unsqueeze(1)

# 创建测试数据窗口，使用反射填充
test_data = data[train_val:]
test_data_windows = create_windows(test_data, window_size=window_size, padding="reflect")
